...
//张桂飞combo下拉框的高度适应修改
var outHeight=document.documentElement.clientHeight;
if(opts.panelHeight > outHeight && document.documentElement.clientHeight < 240)
{
	opts.panelHeight=outHeight-80;
}else{
	if(opts.panelHeight>200)
	{
		panelHeight=200;
	}
}
...
//张桂飞修改combobox下拉框过长问题
if(data){
	if(data.length*18 > document.documentElement.clientHeight && document.documentElement.clientHeight < 240){
		_76e.panel('resize', {
			height : document.documentElement.clientHeight-80
		});
	}else{
		if(data.length*18 > 200){
			_76e.panel('resize', {
				height : 200
			});
		}
